...
public static class MyExtensions
{
    ...
    public static string ToDateMonthFormat(this string date)
    {
        return date.ToDateTime().ToString(RoadFlow.Utility.Config.DateMonthFormat);
    }
    ...
    {
        return dateTime.ToString(RoadFlow.Utility.Config.DateTimeFormatString);
    }
    public static string ToNumberStringTrimEndZero(this decimal s)//
    {
        string st = s.ToString();
        //return st.Contains(".") ? st.TrimEnd('0').TrimEnd('.') : st;
       // if (s.ToString().ToInt() == 0)
         //   return "0";
       //return st.Contains(".") ? st.Trim('0').Trim('.') : st;
        double num = double.Parse(st.Replace(",", "")); return num.ToString("G").Replace(",", "");
